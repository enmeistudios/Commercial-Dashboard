...
Admin Holds count as a paid visit (confirmed with Remy): these are
mainly private classes, so a held spot is real revenue, not empty
capacity. Unavailable Holds (broken equipment etc.) are deliberately
NOT included -- that's lost capacity, not a paid visit. FUTURE
ENHANCEMENT, not built yet: eventually assign an average RPV (revenue
per visit) specifically to Admin Hold spots, since private-class
economics likely differ from a standard drop-in/membership visit.
...
def clean_sessions_df(raw_df):
    ...
    df["Class Date"] = pd.to_datetime(df["Class Date"], errors="coerce", dayfirst=True)
    ...
    # NOTE: Admin Holds was briefly considered for inclusion here (private
    # classes = paid visits, per Remy) but REVERTED pending further
    # validation -- a bulk pattern of 153 rows all showing Marylebone,
    # capacity 2, checked-in 0, Admin Holds 1, repeating at fixed times
    # across many days does NOT look like private-class bookings. Likely
    # a recurring equipment/maintenance hold instead. Do not include
    # Admin Holds in Paid Visits until a genuine private-class example is
    # confirmed (see conversation history).
    df["Paid Visits"] = (
        df["Checked In Reservations"] + df["Late Cancelled Reservations"]
        + df["No Showed Reservations"]
    )
