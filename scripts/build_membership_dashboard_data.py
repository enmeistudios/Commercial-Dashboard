...
def build_customer_log(df):
    ...
    Includes is_switch_cancellation (True if this "cancellation" was
    actually an upgrade/downgrade/studio switch/renewal artifact -- see
    detect_switches) so dashboard-side dropoff/retention analysis can
    correctly exclude plan changes from real churn, same as History does."""
    log_df = df[[
