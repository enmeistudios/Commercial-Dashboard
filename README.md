...
└── docs/
    └── index.html              # the dashboard itself -- single source of truth
```
...
1. `build_membership_dashboard_data.py` pulls membership data from
   ...
   and writes it into the `Membership_Data` and `Membership_History_Detail`
   tabs of a shared Google Sheet.
2. This runs automatically every Monday via GitHub Actions.
3. `docs/index.html` is a single, self-contained HTML file (no build
   step, no npm, no framework) that reads that sheet live via Google's
   public `gviz` endpoint and renders the charts/filters.
4. **GitHub Pages** serves `docs/index.html` directly (Settings -> Pages
   -> Deploy from branch -> `main` -> `/docs`). This requires the repo
   to be public. There is only one copy of the dashboard file -- edit
   `docs/index.html` and nothing else.
